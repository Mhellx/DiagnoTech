...
def gerenciar_clientes(request):
    ...
    if nome_completo:
        clientes = clientes.annotate(full_name=Concat('first_name', Value(
            ' '), 'last_name')).filter(full_name__contains=nome_completo)
...
def alterar_dados_exame(request, exame_id):
    ...
    if requer_senha and (not exame.senha):
        messages.add_message(request, constants.ERROR, 'Para exigir uma senha, gere uma senha primeiro!')
